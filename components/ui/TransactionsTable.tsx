...
const CategoryBadge = ({ category }: CategoryBadgeProps) => {
    ...
    const { 
        ...
        textColor,
        chipBackgroundColor,
     } = transactionCategoryStyles[formattedCategory as keyof typeof transactionCategoryStyles] || transactionCategoryStyles.default;

    return (
        <div className={cn('category-badge', borderColor, chipBackgroundColor)}> 
            <div className={cn('size-2 rounded-full', backgroundColor)} />
            <p className={cn('text-[12px] font-medium', textColor)}>{formattedCategory}</p>
        </div>
    )
}
    ...
    <Table> 
      ...
      <TableBody>
        {transactions.map((t: Transaction) => {
            ...
            return (
                <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} border-b`}>
                    ...
                    <TableCell className='pl-2 pr-10 max-md:hidden'>
                        <CategoryBadge category={t.category ? t.category.replace(/_/g, ' ') : 'Uncategorized'} />
                    </TableCell>
                </TableRow>
            )
        })}
